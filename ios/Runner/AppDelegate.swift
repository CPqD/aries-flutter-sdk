...
@objc class AppDelegate: FlutterAppDelegate {
    ...
    override func application(
        ...
        channel = FlutterMethodChannel(name: channelName, binaryMessenger: rootViewController  as! FlutterBinaryMessenger)
...
extension AppDelegate : AgentDelegate{
    ...
    func sendCredentialEventToFlutter(id: String, state: String) {
        ...
        DispatchQueue.main.async {
            self.channel.invokeMethod("credentialReceived", arguments:data) { (result) in
    ...
    func sendCredentialRevocationToFlutter(id: String) {
        ...
        DispatchQueue.main.async {
            self.channel.invokeMethod("credentialRevocationReceived", arguments:data) { (result) in
                if let resultString = result as? String {
                    print(resultString)
                }
            }
    ...
    func sendProofReceivedToFlutter(id: String, state: String) {
        ...
        DispatchQueue.main.async {
            self.channel.invokeMethod("proofReceived", arguments:data) { (result) in
                if let resultString = result as? String {
                    print(resultString)
                }
            }
    ...
    func sendBasicMessageToFlutter(_ message: BasicMessageRecord) {
        ...
        DispatchQueue.main.async {
            self.channel.invokeMethod("basicMessageReceived", arguments:data) { (result) in
                if let resultString = result as? String {
                    print(resultString)
                }
            }
